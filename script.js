...
document.addEventListener('DOMContentLoaded', function() { 
  ...
  if (contactForm) { 
    contactForm.addEventListener('submit', async function(e) { 
      ...
      // Try to intelligently map form fields to Zappy API format
      const name = formFields.name || formFields.fullName || formFields.fullname || formFields.userName || '';
      const email = formFields.email || formFields.userEmail || formFields.mail || '';
      const phone = formFields.phone || formFields.telephone || formFields.phoneNumber || formFields.tel || null;
      const message = formFields.message || formFields.comments || formFields.comment || formFields.text || '';
      const subject = formFields.subject || formFields.topic || 'Contact Form Submission';
      ...
      if (window.zappyContactFormLoaded) {
        try {
          ...
          const response = await fetch('https://qaapi.zappy5.com/api/email/contact-form', {
            ...
            body: JSON.stringify({
              websiteId: '8c1f0a9e-0bfb-4b88-84f3-6bd6943e9bc5',
              name: name,
              email: email,
              subject: subject,
              message: message,
      ...
      alert('תודה על פניתך! נחזור אליך בהקדם האפשרי.'); 
      contactForm.reset(); 
    }); 
  } 
});
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "מלך הפלאפל משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
